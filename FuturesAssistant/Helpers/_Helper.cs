...
namespace FuturesAssistant.Helpers
{
    public static class _Helper
    {
            ...
            out decimal? yesterdayBalance, out decimal? todayBalance, out decimal? remittanceTotal, out decimal? amount, Guid accountId)
        {
            ...
            int? headerIndex = null, infoIndex = null, 
                funStatusIndex = null, remittanceIndex = null, 
                tradeIndex = null, tradeQQIndex = null, 
                positionIndex = null, positionQQIndex = null;
            ...
            while (currentRowIndex < statementTable.Rows.Count)
            {
                ...
                else if (statementTable.Rows[currentRowIndex][0].ToString().Trim().Contains("期货成交汇总"))
                {
                    tradeIndex = currentRowIndex + 2;
                }
                else if (statementTable.Rows[currentRowIndex][0].ToString().Trim().Contains("期货持仓汇总"))
                {
                    positionIndex = currentRowIndex + 2;
                }
                else if (statementTable.Rows[currentRowIndex][0].ToString().Trim().Contains("期权成交汇总"))
                {
                    tradeQQIndex = currentRowIndex + 2;
                }
                else if (statementTable.Rows[currentRowIndex][0].ToString().Trim().Contains("期权持仓汇总"))
                {
                    positionQQIndex = currentRowIndex + 2;
                }
                currentRowIndex++;
            }
